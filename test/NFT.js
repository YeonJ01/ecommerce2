...
contract('Test MyNFT contract', function(accounts) {
    ...
    it('deploy and mint ERC 721 token', async () => {
        token = await MyNFT.new(name, symbol)
        await token.registerUniqueToken(account1, tokenId1,
            tokenUri1, {from: accounts[0]})
    ...
    it(' check unique id', async () => {
        const duplicateTokenID = token.registerUniqueToken(account2,
            tokenId1, tokenUri2, {from: accounts[0]})
    ...
    it(' safe transfer', async () => {
        const unownedTokenId = token.safeTransferFrom(account2,
            account3, tokenId1, {from: accounts[2]}) //tokenId
        expect(unownedTokenId).to.be.rejectedWith(/VM Exception while processing transaction: revert/)

        const wrongOwner = token.safeTransferFrom(account1,
            account3, tokenId2, {from: accounts[1]})
        expect(wrongOwner).to.be.rejectedWith(/VM Exception while processing transaction: revert/)

        const wrongFromGas = token.safeTransferFrom(account2,
            account3, tokenId2, {from: accounts[1]})
